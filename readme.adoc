...
echo "export TF_VAR_PROJECT_ID=\"$TF_VAR_PROJECT_ID\"" >> ~/.bash_profile
...
to install
Red Hat Openshift Service Mesh on OCP 4.6:

[source,bash]
----
su - crcuser #Enter the word password when you are prompted for it
git clone https://github.com/danielmenezesbr/crc-on-gcp-terraform
cd crc-on-gcp-terraform
...
* ddns_hostname (e.g myopenshift.duck.org )

Sensitive variables must be set in `secrets.tfvars`:

* ddns_login
* ddns_password (leave blank for duck.org)
